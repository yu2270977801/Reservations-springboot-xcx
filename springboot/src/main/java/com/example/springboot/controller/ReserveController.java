...
public class ReserveController {
    ...
    public Result save(@RequestBody Reserve reserve) {
        if (reserve.getId() == null) {
            ...
            //reserve.setUser(TokenUtils.getCurrentUser().getUsername());
        }
    ...
    public Result user(@PathVariable Integer userId) {
        List<Reserve> list = reserveService.list(new QueryWrapper<Reserve>().eq("user_id", userId));
        for (Reserve reserve : list) {
            Integer packageId = reserve.getPackageId();
            Package aPackage = packageService.getById(packageId);
            reserve.setPackageName(aPackage.getName());
        }
